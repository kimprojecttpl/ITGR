...
**Version:** 1.5
**Status:** v1.0–v1.4 shipped and verified on Vercel — this revision specs + implements v1.5 (FY2026 alignment). **Deploy gate: the v1.5 block of `supabase/schema.sql` must be applied before the self-assessment import can run.**
...
- **FY2026 Alignment (v1.5)**
  ...
  - `GET /api/items` tolerates the v1.5 columns being absent, so the frontend/BFF can deploy before the migration is applied without taking the dashboard down.
...
2. ~~NC needs to be a real, visible status, not folded into an existing one~~ — **resolved in v1.5.** The v1.3 decision merged `Not Applicable` into `Not Compliant`; the caveat recorded here at the time was that COE&S might later need to mark a requirement "doesn't apply to us." The FY2026 workbook did exactly that for 8 requirements, and Marubeni's scoring excludes them from the denominator rather than failing them — so v1.5 restores `Not Applicable` as a sixth status alongside `Not Compliant`, and both now exist.
