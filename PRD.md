...
**Migration note (blocking, see § Open Questions):** existing production users hold `admin` / `read_write` / `read_only` roles today (real accounts: `admin`, `lead`, `reviewer` — note "reviewer" is currently just a display name, not yet a role). `read_only` maps cleanly to the new model unchanged. `read_write` does **not** map 1:1 to any single new role — each existing `read_write` user needs to be explicitly assigned `user`, `reviewer`, or `approver` (or more than one, if the org wants one person able to act as both User and Reviewer on different items — not recommended, defeats the segregation-of-duties goal). This requires a stakeholder decision before the schema migration runs, not an automatic mapping.
...
- **Approval Workflow (v1.3)**
  ...
  - **Submit for Approval**: visible to `user`/`admin` on items in `Not Started`, `In Progress`, or `Rejected`. Requires Owner to be filled in (minimum bar — exact required-fields list is an open question). Transitions to `Pending Review`.
  ...
  - **Approver action**: visible to `approver`/`admin` on items in `Pending Approval`. Three choices:
    ...
    - **Complied with Condition** → terminal state `Complied with Condition` (counted as **Partial** for all existing progress %/grade calculations — no changes needed to `progressPct()`/`gradeFor()`). Requires at least one of: an attached evidence document, or an exception comment explaining the condition (exact and/or requirement is an open question below).
  ...
  - The existing `status` field/enum (used by the Overview KPIs, radar chart, and grade panels from § v1.1/v1.2) must keep working without a rewrite. `workflow_state` maps onto it as follows: `Compliant` maps straight through; `Complied with Condition` maps to `Partial`; `Not Compliant` needs its own mapping decision (see Open Questions — today's `Not Applicable` means "doesn't apply," not "fails," so NC likely needs to become a real sixth status value rather than being folded into an existing one).
...
- `item_status` gains: `clickup_url` (text, nullable), `workflow_state` (text, default `Not Started`, the six-plus states from § Functional Requirements).
...
- **How do existing `read_write` users map to `user`/`reviewer`/`approver`** (stakeholder, v1.3, **blocking**): no automatic mapping is safe — needs a named decision per existing account before the role migration runs. See § 4a.
...
- **Minimum required fields to allow "Submit for Approval"** (stakeholder, v1.3, blocking): assumed "Owner filled in" is the floor; should Evidence(note) or the ClickUp Task link also be required before submission is allowed?
- **"Complied with Condition" evidence requirement — AND or OR** (stakeholder, v1.3, blocking): the request says "ต้องใส่เอกสารหลักฐานแนบ และหรือ comment" (must attach a document, and/or comment) — spec'd above as "at least one of the two," but confirm whether an exception comment alone (no file) is ever sufficient, or a file is always mandatory for this specific decision.
- **Does Approver-level "Not Compliant" need a new status value** (engineering, v1.3, blocking): today's `status` enum has no NC value (`Not Applicable` means something different — "doesn't apply," not "fails"). This needs either a new status value (small breaking change to `STATUSES`/`STATUS_TH`/`STATUS_COLOR` etc. in `index.html`, and the `item_status.status` check constraint) or a decision to keep NC only in `workflow_state` and choose an existing display status for it — recommend the former (add NC as a real, visible status) since hiding "failed" behind an existing label undermines the point of the workflow.
...
2. **NC needs to be a real, visible status**, not folded into an existing one (flagged as a blocking open question above) — otherwise the dashboard's own KPIs/grades can't distinguish "failed" from "not applicable" or "not started," which undermines the whole point of adding an Approver decision.
