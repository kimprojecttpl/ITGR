...
// else (including Not Compliant) counts zero, and "Not Applicable" leaves
// the denominator entirely (v1.7 — Marubeni's own scoring excludes N/A
// rather than failing it). If this ever disagrees with the dashboard's own
// Overview tab, that is a bug in this file, not a difference in "how you
// could reasonably compute it."
...
function progressPct(items) {
  ...
  for (const it of items) {
    if (it.status === "Not Applicable") continue;
    total += 1;
...
export default async function handler(req, res) {
  ...
  if (req.query.cat !== undefined) {
    ...
    if (!Number.isInteger(catFilter) || catFilter < 1 || catFilter > 8) {
      await respondError(req, res, token.id, startedAt, 400, "invalid_filter", `"cat=${req.query.cat}" is not a valid category number.`, {
        param: "cat",
